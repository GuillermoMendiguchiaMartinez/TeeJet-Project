...
def waypoints(coordinates,cam_fov_deg, height, overlap,pixel_size=0.5):
    ...
    # Create the local coordinates vector for the real coordinates gotten from GUI. True???
    for coordinate in coordinates:
        coordinates_rel.append(np.ceil(np.array(global2local(coordinate, ref_point))/pixel_size).astype('int'))
